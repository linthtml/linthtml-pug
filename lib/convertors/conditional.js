const { get_node_end_position, index_to_loc, link_nodes } = require("../utils");
...
function generate_block_alternate(node, lines) {
  ...
  alternate_node.children = link_nodes(node.children, alternate_node);
...
function to_condition_node(node, lines) {
  const condition = {
    ...
    // open: ??? // required by linththml indent rules
    loc: node.loc
      ? node.loc
      : {
          start: {
